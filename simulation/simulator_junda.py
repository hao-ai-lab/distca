...
class Simulator:
    ...
    def get_batch_attn_time(self, num_tokens: np.ndarray, tp_degree: int,
        ...
        num_tokens_log = np.log2(num_tokens / 1024)
...
def test(data_path, tokenizer, batch_size,
    ...
    if data_path != "fake":
        ...
        # load batches_16k.json
        with open("data/batches_16k.json", "r") as f:
            doc_dataset_len = json.load(f)
        
        def get_data():
            data_budget = num_tokens_per_data
            doc_lens = []
            for token_count in doc_dataset_len:
                while data_budget < token_count:
                    doc_lens.append(data_budget)
                    yield doc_lens
                    doc_lens = []
    ...
    for sample in range(batch_samples):
        datas = [
            list(get_data()) for _ in range(batch_size)
        ]
...
def main():
    ...
    parser.add_argument("--data_path", type=str, default="fake")
...
"""
python simulator_junda.py --data_path fake --num_tokens_per_data 16 --attn_time data/attn_doc_nh32_hdim128_gqa1.npy --mlp_time data/mlp_time.npy --batch_samples 100
"""