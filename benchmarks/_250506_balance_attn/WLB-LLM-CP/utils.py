...
def compute_per_seq_metadate_combined(
    ...
    torch.Tensor,  # local_k
    torch.Tensor,  # local_v
    list[torch.Tensor],  # cu_seqlens_q_list
    list[torch.Tensor],  # cu_seqlens_k_list
    list[torch.Tensor],  # max_seqlen_q_list
    list[torch.Tensor],  # max_seqlen_k_list
    list[int],  # k_offset_list
    Optional[torch.Tensor]  # local_d_out
]:
    """
    Compute metadata and local tensors for per-sequence column parallel (CP) sharding.
    
    This function handles the sharding of input tensors across multiple GPUs in a column parallel fashion,
    where sequences are split across GPUs. It computes necessary metadata for attention computation
    and returns the local portions of tensors for each GPU.

    Args:
        context_length (int): Total length of the context window across all documents
        q_tensor (torch.Tensor): Query tensor of shape [context_length, num_heads, head_dim]
        k_tensor (torch.Tensor): Key tensor of shape [context_length, num_heads, head_dim]
        v_tensor (torch.Tensor): Value tensor of shape [context_length, num_heads, head_dim]
        doc_lens (list[int]): List of document lengths that sum up to context_length
        cp_size (int): Number of GPUs used for column parallel sharding
        rank (int): Current GPU rank (0 to cp_size-1)
        d_out (Optional[torch.Tensor]): Optional output tensor for backward pass

    Returns:
        Tuple containing:
        - local_q (torch.Tensor): Local portion of query tensor for current GPU
        - local_k (torch.Tensor): Local portion of key tensor for current GPU
        - local_v (torch.Tensor): Local portion of value tensor for current GPU
        - cu_seqlens_q_list (list[torch.Tensor]): Cumulative sequence lengths for queries
        - cu_seqlens_k_list (list[torch.Tensor]): Cumulative sequence lengths for keys
        - max_seqlen_q_list (list[torch.Tensor]): Maximum sequence length for queries
        - max_seqlen_k_list (list[torch.Tensor]): Maximum sequence length for keys
        - k_offset_list (list[int]): Offset indices for key tensor chunks
        - local_d_out (Optional[torch.Tensor]): Local portion of output tensor if provided
    """
