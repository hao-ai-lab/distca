...
def run_allgather(local_rank, world_size, iters, tensor_elems, ret):
    ...
    for _ in range(iters):
        ...
        torch.cuda.synchronize()
        durations_ms.append(start.elapsed_time(end))  # avg per iteration

    elapsed_ms = sum(durations_ms) / iters

    # Gather all elapsed times to rank 0
    all_elapsed_ms = [torch.zeros(1, device="cuda") for _ in range(world_size)]
    torch.distributed.all_gather(all_elapsed_ms, torch.tensor([elapsed_ms], device="cuda"))

    # Report all gathered times
    if local_rank == 0:
...
def gpu_func_maping(gpu_type: str, world_size: int, op: str):
    func = globals()[f"{op}_worker_{gpu_type}_{world_size}"]
    return func
