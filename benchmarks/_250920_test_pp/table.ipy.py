...
import os
import json
root_path = "/mnt/weka/home/yonghao.zhuang/jd/d2/benchmarks/_250920_test_pp/logs.v10-large-scale-pp"
...
df = df.sort_values(by=[
    "nodes", "total_batch_size", "num_tokens", "mode", "average_duration", "cp_size",
], ascending=False)
df
...
wlb_groups_best = df[
    df['mode'] == 'wlbllm'
].groupby(["model_size", "nodes", "total_batch_size", "num_tokens", "dataset"])['average_duration'].min().reset_index()
wlb_groups_best
# %%
d2_groups_best = df[
    df['mode'] == 'd2'
...
merged_wlb_vs_d2 = pd.merge(wlb_groups_best, d2_groups_best, on=['model_size', 'nodes', 'total_batch_size', 'num_tokens', 'dataset'], how='left', suffixes=('_wlb', '_d2'))
...
front_columns = ['model_size', 'nodes', 'num_tokens', 'total_batch_size', 
'speedup', 'dataset',
