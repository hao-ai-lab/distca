...
class PerDocumentCPAttention(torch.autograd.Function):
    ...
    def forward(
        ...
        gather_k_list = [torch.randn_like(local_k) for _ in range(cp_size)]
        gather_v_list = [torch.randn_like(local_v) for _ in range(cp_size)]
...
if __name__ == "__main__":
    ...
    print_doc_cp_attn_network_table(
            head_dim = 128,
        num_qo_heads = 32,
        num_kv_heads = 8,
    )
