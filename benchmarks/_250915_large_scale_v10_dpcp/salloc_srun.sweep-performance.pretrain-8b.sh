...
export OUTPUT_DIR_PREFIX="/mnt/weka/home/yonghao.zhuang/jd/d2/benchmarks/_250915_large_scale_v10_dpcp/logs.v4-sweep-perf-pretrain-8b"
...
for config in \
   ...
   "1 1  4  524288  8 32  wlbllm" \
    ...
    if [ "$mode" == "d2" ]; then
       for tolerance_factor in 0.05 0.1 0.2; do
            # Run d2 mode with all on
            export MODE=d2
            export MIN_TOLERANCE_FACTOR=$tolerance_factor
            export OUTPUT_DIR_SUFFIX_ADDON="-tol${tolerance_factor}-${MODEL_PATH_normalized}"
