...
export OUTPUT_DIR_PREFIX="/mnt/weka/home/yonghao.zhuang/jd/d2/benchmarks/_250915_large_scale_v10_dpcp/logs.v1-memory-pretrain-8b"
...
# "1 1 8 131072 2 8" \
#     "0 0 8 131072 2 8" \
# "1 1 16 262144 4 32 d2" \
# selective_ckpt resend_qkv batch_size num_tokens elongate_factor nnodes mode

# "1 1 8 524288 16 32 d2" \
# "1 1 4 524288 16 32 d2" \
# "1 1 2 524288 16 32 d2" \
# "1 1 1 524288 16 32 d2" \
# "1 1 4 524288 16 16 d2" \
# "1 1 2 524288 16 8 d2" \
# "1 1 8 262144 4 16 d2" \
   # s r b   t   e  N
for config in \
   "1 1 2 262144 2 8 d2" \
   "1 1 4 262144 4 16 d2" \
   "1 1 8 262144 8 32 d2" \
   "1 1 4 131072  2 8 d2" \
   "1 1 8 131072  4 16 d2" \
   "1 1 16 131072 8 32 d2" \
    ; do
    ...
    export MODEL_PATH_normalized=$(echo $model_path | sed 's/\//--/g')
    export NNODES=$nnodes
    ...
    if [ "$mode" == "d2" ]; then
        ...
        export MODE=d2
        export OUTPUT_DIR_SUFFIX_ADDON="-normal-${MODEL_PATH_normalized}"
    ...
    # # Run d2 mode sweeping some tolerance factors
    # for tolerance_factor in 0.2; do
    #     export MODE=d2
    #     export MIN_TOLERANCE_FACTOR=$tolerance_factor
    #     export OUTPUT_DIR_SUFFIX_ADDON="-tol${tolerance_factor}-${MODEL_PATH}"
    #     eid="d2-cp1-n${NNODES}-b${BATCH_SIZE}-t${NUM_TOKENS}"
    #     if [[ "$SUCCESS_EIDS" =~ "$eid" ]]; then
    #         echo "🟢 Skip: $eid"
    #         continue
