...
class PerSequenceCPAttention(torch.autograd.Function):
    ...
    def backward(ctx, d_out_cat):
        ...
        for i, (d_out, q_len, out, lse, kv_k, kv_v, cu_q, cu_k, max_q, max_k) in enumerate([
            ...
            # Declaration: flash_attn::_flash_attn_varlen_backward(Tensor dout, Tensor q, Tensor k, Tensor v, Tensor out, Tensor softmax_lse, Tensor(a6!)? dq, Tensor(a7!)? dk, Tensor(a8!)? dv, 
            # Tensor cu_seqlens_q, Tensor cu_seqlens_k, SymInt max_seqlen_q, SymInt max_seqlen_k, 
            # float dropout_p, float softmax_scale, bool causal, SymInt window_size_left, SymInt window_size_right, float softcap, Tensor? alibi_slopes, bool deterministic, Tensor? rng_state=None, bool zero_tensors=False) -> Tensor
            ...
            _ = _flash_attn_varlen_backward(
                ...
                0.0, ctx.softmax_scale, True, -1,-1, softcap, None, False, None
            )
