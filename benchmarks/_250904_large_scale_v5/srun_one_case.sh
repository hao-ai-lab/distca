# 
# Usage
#   SHOULD_NSYS_PROFILE=1 SHOULD_PROFILE_MEMORY=1 BATCH_SIZE=1 NUM_LAYERS=4 NUM_TOKENS=65536 ELONGATE_FACTOR=1 MAX_SAMPLE_ID=2 bash test_e2e_combined.onesrun.sh
...
srun -N $nnodes -G $ngpus --ntasks-per-node=1 --jobid=$jobid -w "$head_node" \
    ...
    bash -lc "
    set -x
    exec $nsys_str torchrun --nnodes=$nnodes --nproc_per_node=8 --rdzv_backend=c10d --rdzv_endpoint=$head_node:29500 --rdzv_id=d2-n$nnodes --max_restarts=0 test_e2e_combined.py --model-path deepseek-ai/DeepSeek-R1-Distill-Llama-8B --mode d2 --replan-iter 0 --batch-size $BATCH_SIZE --num-nodes $nnodes --num-gpus-per-node 8 --num-layers $NUM_LAYERS --max-sample-id $MAX_SAMPLE_ID --tp-size 8 --cp-degree 1 --up-sample-factor 4 --num-tokens $NUM_TOKENS --elongate-factor $ELONGATE_FACTOR --filter-threshold 65536 --filter-ratio 0.50 --output-dir ${OUTPUT_DIR} $EXTRA_ARGS
    "
