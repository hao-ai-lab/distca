...
if [ ${ENABLE_NSYS} -eq 1 ]; then
  nsys_str="nsys profile --show-output=true --capture-range=cudaProfilerApi --capture-range-end=stop --force-overwrite=true -o ${NSYS_PATH}/%h.nsys-rep --sample="process-tree"  -t cuda,nvtx,osrt --backtrace=lbr --cudabacktrace=true"
